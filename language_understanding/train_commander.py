...
from .ground import date_ground, loc_ground, is_loc_name, time_ground, date_to_time, price_ground
from .train_ground import type_ground, trainnum_ground, station_ground, seats_ground
...
SLOTS = ['start_city', 'arrival_city',  'start_date', 'arrival_date', 'start_time',\
'price','cost_relative', 'arrval_time','train_type','seats']
...
TRAIN_TYPICAL_SLOTS = ['train_type','seats','price']
...
class TrainCommander(object):
    ...
    def get_reply_with_lexical(self, sent, words, postags, nes, context):
        ...
        self.fill_station(sent)
        self.fill_seats(sent)
        self.construct_regularize_slots()
        self.fill_slot_accord_pre_info() 
    ...
    '''
    每轮对话pre_info的alive减1，pre_info里只存规范化的
    '''
    ...
    def fill_seats(self, sent):
        seats = seats_ground(sent)
        if seats != None:
            self.slots['seats'] = seats

    def fill_station(self, sent):
        stations = station_ground(sent)
        if stations != None:
            if stations.has_key('originStation'):
                self.slots['originStation'] = stations['originStation']
            if stations.has_key('terminalStation'):
                self.slots['terminalStation'] = stations['terminalStation']
    ...
    def construct_reply(self):
        ...
        # 如果必须的slot都有了
        # 若之前完成过意图且有更精确的筛选范围
        if self.context.has_key('last_finish_intent') and self.context['last_finish_intent'] == "train":
            for k in TRAIN_TYPICAL_SLOTS:
                if k not in self.slots and k in self.context['last_finish_slot']:
                    if k in TRAIN_TYPICAL_SLOTS:
                        continue
        ...
        if info['status'] == 1:
            ...
            if 'start_time' in self.slots or 'arrival_time' in self.slots:
                ...
                if 'arrival_time' in self.slots:
                    ...
                    for t in trains[0:]:
                        at = self.hash_time(t.arrivalTime)
            ...
            if 'train_type' in self.slots:
                print self.slots
                traintype = self.slots['train_type']
                for t in trains[0:]:
                    print t
                    train_type = t.trainNo
                    if traintype not in train_type:
                        trains.remove(t)
            ...
            if 'price' in self.slots and self.slots['price'].has_key('num'):
                for t in trains:
                    for s in t.prices:
                        if deletePrice(self.slots,s.value):
                            t.prices.remove(s)
                    if len(t.prices) == 0:
                        trains.remove(t)
            ...
            reply = u"目前只能查询4月18日至5月17日的火车票哦,请问您要查哪天的？"
        ...
        if DEBUG:
            ...
            context[CONTEXT_SLOTS] = self.slots
        print 'IN TRAIN_COMMANDER: OUTPUT: ',context
        return STATUS_SUCCESS, reply.encode('utf-8'), context
    ...
    def make_reply_title(self, start_date, start_city, arrival_city):
        ...
        day = day.lstrip('0')
        return u"以下是我找到的%s月%s日从%s到%s的火车票" % (month, day, start_city, arrival_city)

    def get_ticket_info(self, slots):
        retrieval = TrainRetrieval(slots)
